...
algo[["process_instance"]] <- 
  CJ(contrast = "define_contrast()",
     ...
     analysis = "co-primary",
     regu = "c(1,1/2,1/4)",
     pars = c("list()", "list(type='wild')", "list(lfc_pr=0.5)", "list(lfc_pr=0)")) %>% 
  filter(! (adjustment!="bootstrap" & pars == "list(type='wild')")) %>% 
  filter(! (adjustment!="mbeta" & pars %in% c("list(lfc_pr=0.5)", "list(lfc_pr=0)")))
